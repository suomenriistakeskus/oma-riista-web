...
[ -f "/tmp/palstaalue.pgdump" ] && sha1sum -s -c /tmp/palstaalue.sha || wget "${BUCKET_PREFIX}/palstaalue.pgdump" -O /tmp/palstaalue.pgdump
echo "67248c71dde315890d0fa6a03376c9b976a43a0c  /tmp/vesialue.pgdump" > /tmp/vesialue.sha
[ -f "/tmp/vesialue.pgdump" ] && sha1sum -s -c /tmp/vesialue.sha || wget "${BUCKET_PREFIX}/vesialue.pgdump" -O /tmp/vesialue.pgdump
...
[ -f "/tmp/rhy_data.zip" ] && sha1sum -s -c /tmp/rhy_data.sha || wget "${BUCKET_PREFIX}/rk-rhy-v1-5.zip" -O /tmp/rhy_data.zip
...
[ -f /tmp/hta_data.zip ] && sha1sum -s -c /tmp/hta_data.sha || wget "${BUCKET_PREFIX}/hta-v1.zip" -O /tmp/hta_data.zip
