...
public class OrganisationEventApiResource {
    ...
    public List<CalendarEventType> readEventTypes() {
        return new ArrayList<>(CalendarEventType.activeCalendarEventTypes());
    }
    ...
    @CacheControl(policy = CachePolicy.NO_CACHE)
    @GetMapping(value = "/{orgId:\\d+}/events/year/{year:\\d+}")
    public List<CalendarEventDTO> listEventsByYear(@PathVariable final long orgId,
                                                   @PathVariable final int year) {
        return organisationCrudFeature.listEventsByYear(orgId, year);
    }
    ...
    public CalendarEventDTO createEvent(@RequestBody @Validated final CalendarEventDTO dto,
        ...
        Preconditions.checkArgument(CalendarEventType.activeCalendarEventTypes().contains(dto.getCalendarEventType()));
