...
@Service
public class PersonalAreaUnionZoneCache {
    private final TransactionTemplate transactionTemplate;
    private final ActiveUserService activeUserService;
    private final PersonalAreaUnionRepository personalAreaUnionRepository;
    ...
    @Autowired
    public PersonalAreaUnionZoneCache(final PlatformTransactionManager transactionManager,
                                      final ActiveUserService activeUserService,
                                      final PersonalAreaUnionRepository personalAreaUnionRepository) {
        this.transactionTemplate = new TransactionTemplate(transactionManager);
        this.transactionTemplate.setReadOnly(true);
        this.activeUserService = activeUserService;
        this.personalAreaUnionRepository = personalAreaUnionRepository;
    ...
    private boolean hasReadPermission(final Tuple2<String, Long> key) {
        final PersonalAreaUnion area = personalAreaUnionRepository.getOne(key._2());
        return activeUserService.checkHasPermission(area, EntityPermission.READ);
    }

    private Long getInternalZoneId(final Long areaId) {
        return Optional
                .ofNullable(personalAreaUnionRepository.getOne(areaId))
                .map(PersonalAreaUnion::getHarvestPermitArea)
