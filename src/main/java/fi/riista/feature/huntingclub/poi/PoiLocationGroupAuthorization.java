...
@Component
public class PoiLocationGroupAuthorization extends AbstractEntityAuthorization<PoiLocationGroup> {

    @Resource
    private UserAuthorizationHelper userAuthorizationHelper;

    public PoiLocationGroupAuthorization() {
    ...
    protected void authorizeTarget(@Nonnull final AuthorizationTokenCollector collector,
                                   @Nonnull final PoiLocationGroup poi,
                                   @Nonnull final UserInfo userInfo) {
        userAuthorizationHelper.getPerson(userInfo).ifPresent(activePerson -> {
            final HuntingClub club = poi.getPoiIdAllocation().getClub();
