...
@Service
public class AdminMooselikeHuntingMetricsService {
    ...
    public List<AdminMooselikeHuntingMetrics> computeMooselikeHuntingMetrics() {
        ...
        final Map<Long, AdminMooselikeHuntingMetrics> permitMetrics = countPermitsAndMooseDataCardGroups(huntingYear);

        return speciesList.stream()
                .map(gameSpecies -> {
                    final AdminMooselikeHuntingMetrics metrics = new AdminMooselikeHuntingMetrics();
                    metrics.setSpeciesName(gameSpecies.getNameFinnish());
                    ...
                    final AdminMooselikeHuntingMetrics permitCounts = permitMetrics.get(speciesId);

                    if (permitCounts != null) {
                        metrics.setOpenPermitCount(permitCounts.getOpenPermitCount());
                        metrics.setClosedPermitCount(permitCounts.getClosedPermitCount());
                        metrics.setModeratorClosedPermitCount(permitCounts.getModeratorClosedPermitCount());
                        metrics.setMooseDataCardGroupCount(permitCounts.getMooseDataCardGroupCount());
                    }
    ...
    private Map<Long, Integer> countObservations(final int huntingYear,
                                                 final Collection<Long> includedGameSpeciesIds) {
        final SQObservation observation = SQObservation.gameObservation;
    ...
    private Map<Long, AdminMooselikeHuntingMetrics> countPermitsAndMooseDataCardGroups(final int huntingYear) {
        ...
        return namedParameterJdbcTemplate.query(queryStr, queryParams, (resultSet, i) -> {
            final AdminMooselikeHuntingMetrics metrics = new AdminMooselikeHuntingMetrics();
            metrics.setSpeciesId(resultSet.getLong("species_id"));
            ...
            return metrics;
        }).stream().collect(toMap(AdminMooselikeHuntingMetrics::getSpeciesId, identity()));
    }
}
