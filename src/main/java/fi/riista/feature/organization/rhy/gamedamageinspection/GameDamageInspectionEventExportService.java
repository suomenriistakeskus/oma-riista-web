...
public class GameDamageInspectionEventExportService {
    ...
    public List<GameDamageInspectionEventSummaryDTO> getGameDamageInspectionEventSummary(final int year,
        ...
        return rhyToEvent.entrySet().stream()
                .map(entry -> {
                    ...
                    Iban iban = null;
                    if (rhyIdToStatistics.get(rhy.getId()) != null) {
                        iban = rhyIdToStatistics.get(rhy.getId()).getOrCreateBasicInfo().getIban();
                    }
                    if (iban == null) {
                        iban = annualStatisticsService.getAnnualStatisticsResolver(rhy, year).getIbanFromPreviousYear();
                    }
    ...
    public GameDamageInspectionEventExportDTO getGameDamageInspectionEventExport(final long rhyId,
        ...
        if (annualStatistics != null) {
            iban = annualStatistics.getOrCreateBasicInfo().getIban();
        }
        if (iban == null) {
            iban = annualStatisticsService.getAnnualStatisticsResolver(rhy, year).getIbanFromPreviousYear();
        }
    ...
    private Map<Long, GameSpecies> getIdToGameSpeciesMapping(final List<GameDamageInspectionEventDTO> dtos) {
