...
import fi.riista.feature.organization.rhy.subsidy2019.SubsidyBatch2019InfoDTO;
import fi.riista.feature.organization.rhy.subsidy2019.compensation.SubsidyAllocation2019CompensationResultDTO;
import fi.riista.feature.organization.rhy.subsidy2019.compensation.SubsidyCompensation2019OutputDTO;
import fi.riista.feature.organization.rhy.subsidy2019.excel.RhySubsidy2019ExcelModel.AggregatedSubsidyAllocation;
...
// Specialization for year 2019
public class RhySubsidy2019ExcelView extends AbstractXlsxView {
    ...
    public RhySubsidy2019ExcelView(@Nonnull final AllSubsidyAllocation2019InfoDTO allAllocationInfo,
        ...
        this.allRkaAllocations =
                RhySubsidy2019ExcelModel.groupRhyAllocationsByRka(allAllocationInfo.getCalculatedRhyAllocations());
    ...
    private void addHeaderRows(final ExcelHelper sheetWrapper) {
        ...
        if (isFirstSubsidyBatchAlreadyGranted) {
            sheetWrapper
                    .appendRow()
                    .appendTextCellBold(i18n.getTranslation("RhySubsidy2019Excel.totalSubsidyAmountGrantedInBatch1"))
    ...
    private void addColumnTitleRow(final ExcelHelper sheetWrapper) {
        ...
        sheetWrapper
                .appendTextCellBold(i18n.getTranslation("RhySubsidy2019Excel.statisticsBasedSubsidyAmountOfBatch2"))
                .appendTextCellBold(i18n.getTranslation("RhySubsidyExcel.stage2RemainderEuros"));

        if (isFirstSubsidyBatchAlreadyGranted) {
            sheetWrapper
                    .appendTextCellBold(i18n.getTranslation("RhySubsidy2019Excel.subsidyAmountOfBatch2BeforeCompensation"))
                    .appendTextCellBold(i18n.getTranslation("RhySubsidy2019Excel.subsidyAmountGrantedInBatch1"));
        }
    ...
    private void appendRhySubsidyAllocation(final ExcelHelper sheetWrapper,
                                            final SubsidyAllocation2019Stage4DTO allocation) {

        appendCommonSubsidyAllocationData(sheetWrapper, allocation);

        final SubsidyBatch2019InfoDTO subsidyBatchInfo = allocation.getSubsidyBatchInfo();
        ...
        final SubsidyRoundingDTO stage4Rounding = allocation.getStage4Rounding();

        if (compensationResult.getNumberOfRounds() > 0) {
            final String rhyCode = allocation.getOrganisation().getOfficialCode();

            final List<SubsidyCompensation2019OutputDTO> compensationResults =
                    compensationResult.getCompensationOutputs(rhyCode);

            for (final SubsidyCompensation2019OutputDTO compResult : compensationResults) {
    ...
    private void appendAggregateSubsidyAllocation(final ExcelHelper sheetWrapper,
        ...
        final SubsidyAllocation2019Stage4DTO summary = aggregate.summary;
        ...
        if (aggregate.isSummaryOfAllRhys) {
            final SubsidyBatch2019InfoDTO subsidyBatchInfo = summary.getSubsidyBatchInfo();
