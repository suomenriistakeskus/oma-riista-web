...
import static fi.riista.feature.permit.application.SpeciesPeriodRestrictions.getSpeciesPeriodRestriction;
import static java.util.Objects.requireNonNull;

public class DeportationSpeciesPeriodDTO implements HasBeginAndEndDate {

    public DeportationSpeciesPeriodDTO() {
    }

    public DeportationSpeciesPeriodDTO(final @Nonnull HarvestPermitApplicationSpeciesAmount speciesAmount) {
    ...
    private boolean periodLengthInBounds() {
        final LocalDate maxEnd = Optional.ofNullable(getSpeciesPeriodRestriction(gameSpeciesCode))
                ...
                .orElse(beginDate.plusYears(1))
                .minusDays(1); // Minus one day since begin date is also included in the period
