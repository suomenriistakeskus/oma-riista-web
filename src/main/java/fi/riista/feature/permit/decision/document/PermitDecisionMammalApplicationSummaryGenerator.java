...
public class PermitDecisionMammalApplicationSummaryGenerator {
    ...
    private void createPeriods(final StringBuilder sb) {
        ...
        if (model.getExtendedPeriodGrounds() != null) {
            cell2(sb,
                    i18nKey("mammal.application.reason.extendedPeriodGrounds"),
                    i18nKey("mammal.application.reason.extendedPeriodGrounds." + model.getExtendedPeriodGrounds()));
        }

        if (isNotBlank(model.getProtectedAreaName())) {
            cell2(sb,
                    i18nKey("mammal.application.reason.protectedAreaName"),
                    model.getProtectedAreaName());
        }

        if (isNotBlank(model.getExtendedPeriodDescription())) {
            cell2(sb,
                    i18nKey("mammal.application.reason.extendedPeriodGroundsDescription"),
                    model.getExtendedPeriodDescription());
        }
