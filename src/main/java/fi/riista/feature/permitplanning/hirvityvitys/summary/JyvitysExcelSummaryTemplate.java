...
public class JyvitysExcelSummaryTemplate implements JyvitysExcelCellAddressManipulation {
    ...
    public static final int SHEET_COLUMN_COUNT = 17;
    ...
    public JyvitysExcelSummaryTemplate(final Workbook workbook,
        ...
        checkState(workbook.getNumberOfSheets() == 0,
                "Summary template must be applied on a empty workbook.");
    ...
    public void initializeStaticContent() {
        ...
        sheet.groupColumn(SUMMARY_APPLICATION_AMOUNT.getColumnNumber() + 1,
                SUMMARY_SUGGESTION_TOTAL.getColumnNumber() - 1);
        sheet.setColumnGroupCollapsed(SUMMARY_APPLICATION_AMOUNT.getColumnNumber() + 1, true);

        // Freeze pane to keep headers and appicant names visible
        sheet.createFreezePane(SUMMARY_APPLICANT.getColumnNumber() + 1, APPLICATION_START_ROW);
    }

    public JyvitysExcelSummaryTemplate applyFormulas() {
        // Zero-based
        checkState(sheet.getLastRowNum() > STATIC_CONTENT_HEIGHT_ROWS - 1,
                "Application rows must be inserted before formulas");
        checkState(workbook.getNumberOfSheets() > 1,
                "Verotuslohko sheets must be inserted before formulas");
    ...
    private void applyValueAndStyle(final JyvitysExcelSummaryStaticField field) {
        ...
        checkNotNull(cell, "Cannot find cell for field " + field);
    ...
    private static void applyBorders(CellRangeAddress address, final PropertyTemplate propertyTemplate) {
