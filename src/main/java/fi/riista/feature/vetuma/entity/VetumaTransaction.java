...
import fi.riista.feature.account.registration.InvalidRegistrationStatus;
...
public class VetumaTransaction extends BaseEntity<String> {
    public static final int MAX_TRANSACTION_ID_LENGTH = 19;
    ...
    @Column(name = "vetuma_transaction_id", nullable = false, length = 255)
    public String getId() {
        return id;
    }
    ...
    public VetumaTransaction(final EmailToken emailToken,
        ...
        this.id = UUID.randomUUID().toString().replaceAll("-", "").substring(0, MAX_TRANSACTION_ID_LENGTH);
    ...
    public void setStatusSuccess(final String responseSo, final SystemUser user) {
        assertTransactionStatus(VetumaTransactionStatus.INIT);
        this.status = VetumaTransactionStatus.SUCCESS;
        this.responseSo = responseSo;
        this.endTime = DateUtil.now();
        this.user = user;
    }

    public void setStatusError(final String responseSo) {
        assertTransactionStatus(VetumaTransactionStatus.INIT);
    ...
    public void setStatusMACError(final String responseSo) {
        assertTransactionStatus(VetumaTransactionStatus.INIT);
        this.status = VetumaTransactionStatus.MAC_ERROR;
        this.responseSo = responseSo;
        this.endTime = DateUtil.now();
    }

    public void setStatusTimeout(final String responseSo) {
        assertTransactionStatus(VetumaTransactionStatus.INIT);
    ...
    public void setStatusCancel(final String responseSo) {
        assertTransactionStatus(VetumaTransactionStatus.INIT);
        this.status = VetumaTransactionStatus.CANCELLED;
        this.responseSo = responseSo;
        this.endTime = DateUtil.now();
    }
