...
@Component
public class LupahallintaPermitImportMailHandler {

    private static final Logger LOG = LoggerFactory.getLogger(LupahallintaPermitImportMailHandler.class);
    ...
    private void sendMail(String body, Set<String> rkaCodes) {
        if (runtimeEnvironmentUtil.isProductionEnvironment()) {
            mailService.send(MailMessageDTO.builder()
                    ...
                    .withScheduledTimeAfter(Duration.standardMinutes(1))
                    .withRecipients(listReceivers(rkaCodes))
                    .withSubject(MAIL_SUBJECT)
                    .appendBody(body)
                    .build());
        } else {
            LOG.info("Not a production environment, not sending any mail.");
        }
    }
    ...
    protected static Set<String> collectRkaOfficialCodes(List<HarvestPermitImportResultDTO.PermitParsingError> allErrors) {
        return allErrors.stream()
                ...
                .map(LupahallintaPermitImportMailHandler::resolveRkaOfficialCode)
                .collect(toSet());
    }
