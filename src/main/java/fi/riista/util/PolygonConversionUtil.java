...
public class PolygonConversionUtil {
    ...
    public static org.locationtech.jts.geom.Geometry geoJsonToJava(
    ...
    private static org.locationtech.jts.geom.Geometry makeValid(org.locationtech.jts.geom.Geometry geometry) {
    ...
    public static org.locationtech.jts.geom.Geometry geoJsonToJavaPossiblyInvalid(
    ...
    public static GeoJsonObject javaToGeoJSON(@Nonnull final org.locationtech.jts.geom.Geometry geometry) {
    ...
    // GeoJSON: Polygon
    private static org.locationtech.jts.geom.Polygon geoJsonPolygonToJava(
    ...
    private static org.locationtech.jts.geom.Geometry geoJsonMultiPolygonToJava(
            final MultiPolygon multiPolygon, final GISUtils.SRID srid) {

        final org.locationtech.jts.geom.Polygon[] polygons = multiPolygon.getCoordinates().stream()
        ...
        return polygons.length == 0
                ? null
                : new org.locationtech.jts.geom.MultiPolygon(polygons, GISUtils.getGeometryFactory(srid));
    }

    private static org.locationtech.jts.geom.Polygon coordsToJsonPolygon(
    ...
    private static org.locationtech.jts.geom.Geometry geoJsonGeometryCollectionToJava(
            final GeometryCollection geometryCollection, final GISUtils.SRID srid) {

        final org.locationtech.jts.geom.Geometry[] geometries = geometryCollection.getGeometries().stream()
                .map(geom -> PolygonConversionUtil.geoJsonToJava(geom, srid))
                .toArray(org.locationtech.jts.geom.Geometry[]::new);

        return geometries.length == 0
                ? null
                : new org.locationtech.jts.geom.GeometryCollection(geometries, GISUtils.getGeometryFactory(srid));
    }
    ...
    // Java: Polygon
    private static Polygon javaPolygonToGeoJson(final org.locationtech.jts.geom.Polygon geometry) {
    ...
    // Java: MultiPolygon
    private static MultiPolygon javaMultiPolygonToGeoJson(final org.locationtech.jts.geom.MultiPolygon geometry) {
        ...
        for (int i = 0; i < geometry.getNumGeometries(); i++) {
            ...
            if (subGeometry instanceof org.locationtech.jts.geom.Polygon) {
                multiPolygon.add(javaPolygonToGeoJson(org.locationtech.jts.geom.Polygon.class.cast(subGeometry)));
            }
        }
