...
public class MobileGameDiaryApiResourceTest {

    @Mock
    private MobileHarvestFeature harvestFeature;

    @Mock
    private MobileObservationFeature observationFeature;

    @InjectMocks
    MobileGameDiaryApiResource api;
    ...
    private void testCreateHarvest(Long id, Integer rev) {
        ...
        verify(harvestFeature).createHarvest(captor.capture());
    ...
    private void testCreateObservation(Long id) {
        ...
        verify(observationFeature).createObservation(captor.capture());
        assertNull(captor.getValue().getId());
    }

    @Test
    public void deleteHarvest_ExceptionsAreCaught() {
        doThrow(new RuntimeException("Whatever reason for delete to fail")).when(harvestFeature).deleteHarvest(anyLong());
        api.deleteHarvest(1L);
    }

    @Test
    public void deleteObservation_ExceptionsAreCaught() {
        doThrow(new RuntimeException("Whatever reason for delete to fail")).when(observationFeature).deleteObservation(anyLong());
